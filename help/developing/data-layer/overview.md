...
截止到核心组件发行版本 2.9.0，Data Layer 作为 AEM Client Library 随核心组件分发，无需安装。 由 [AEM 项目原型 v 生成的所有项目。默认情况下，24+](/help/developing/archetype/overview.md) 包含一个激活的数据层。
...
>[!NOTE]
>
>此架构是通用的组件项架构，用作多个核心组件的基本模式。这并不保证每个列出的组件都会在每一项上填充所有这些字段。
