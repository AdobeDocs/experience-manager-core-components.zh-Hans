...
1. [安装核心组件](/help/get-started/using.md#download-and-install)
   * 对于AEM as a Cloud Service项目，核心组件在默认情况下可用，无需额外安装。
   * 对于内部部署和AMS项目，您可以[从GitHub](https://github.com/adobe/aem-core-wcm-components/releases/latest)下载核心组件的最新内容包，并将其安装在AEM环境中。
   * 如果您的内部部署或AMS项目使用的核心组件版本低于2.14.0，则还必须在GitHub上安装作为版本一部分的AMP扩展。
1. 将组件`resourceSuperType`指向`core/wcm/extensions/amp/components/page/v1/page`。
   * 如果您将[AEM项目原型](/help/developing/archetype/using.md)作为推荐的最佳实践用于您的项目并选择[选项来启用AMP支持，](https://github.com/adobe/aem-project-archetype/tree/develop)这将自动为您完成。
...
仅当`resourceSuperType`设置正确以支持AMP时，UI中才会显示这些选项。 默认的WKND示例内容未设置`resourceSuperType`，因此AMP的选项在UI中不可见。
