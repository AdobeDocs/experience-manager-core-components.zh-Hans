...
AEM Project Archetype创建了一个基于最佳实践的最小化Adobe Experience Manager项目，作为您自己的AEM项目的起点。 使用此原型时必须提供的属性允许您指定此项目所有部分的名称，并控制某些可选特征。

## 为什么使用原型{#why-use-the-archetype}

使用AEM Project Archetype，您只需按几下键，即可构建基于最佳实践的AEM项目。 通过使用原型，所有部分都已到位，这样，尽管最终的项目最小，但它已实现AEM的所有[关键功能](#what-you-get)，因此您只需在顶部构建并扩展。

当然，成功的AEM项目中会包含许多元素，但使用AEM Project Archetype是一个可靠的基础，强烈建议任何AEM项目使用它。

## 入门 {#getting-started}

项目原型使得在AEM上开始开发变得很容易。 您可以通过多种方式执行第一步。

* WKND教程 — 有关在AEM上进行开发（包括如何利用原型）的精彩介绍，请参阅[AEM Sites入门 — WKND教程](https://docs.adobe.com/content/help/en/experience-manager-learn/getting-started-wknd-tutorial-develop/overview.html)，以获得一个实际示例，该示例将指导您逐步使用原型来实施一个简单项目。
...
* 自行下载和开始!  — 您可以轻松下载GitHub上可用的当前项目原型，并按照[下面的简单步骤创建您的第一个项目。](#how-to-use-the-archetype)

## 原型{#what-you-get}

AEM Archetype由模块组成：

* **[核心](core.md)**:是一个Java捆绑包，包含所有核心功能(如OSGi服务、侦听器和调度程序)，以及与组件相关的Java代码(如servlet和request过滤器)。
...
使用Maven构建项目会创建可部署到AEM的对象（包和OSGi包）。 其他Maven命令和用户档案可用于将项目对象部署到AEM实例。

### 创建项目 {#create-project}

要开始使用，您最简单的方法是使用[AEM Eclipse扩展](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/implementing/developer-tools/eclipse.html)，然后按照“新建项目”向导选择&#x200B;**AEM多模块项目**&#x200B;以使用已发布版本的原型。
...
仅为非云aem版本添加核心组件依赖关系，因为核心组件是作为云为AEM提供的OOTB
服务。
...
| `aemVersion` | `6.5.0` | 目标 AEM版本(对于[AEM，可以`cloud`作为Cloud Service](https://docs.adobe.com/content/help/en/experience-manager-cloud-service/landing/home.html);或`6.5.0`，或`6.4.4`（适用于[Adobe Managed Services](https://github.com/adobe/aem-project-archetype/tree/master/src/main/archetype/dispatcher.ams)或内部部署）。 |
...
| `frontendModule` | `none` | 包括一个Webpack前端构建模块，用于生成常规站点的客户端库(可以是`general`或`none`;对于实现[SPA Editor](https://docs.adobe.com/content/help/en/experience-manager-cloud-service/implementing/headless/spa/introduction.html)的单页应用程序，可以是`angular`或`react`。 |
...
>[!NOTE]
>
> 如果首次以交互模式执行原型，则无法更改具有默认值的属性（有关更多详细信息，请参阅[ARCHETYPE-308](https://issues.apache.org/jira/browse/ARCHETYPE-308)）。 当结束时的属性确认被拒绝且调查表重复时，或通过在命令行中传递参数(例如，`-DoptionIncludeExamples=n`)。
...
| `autoInstallPackage` | 将包含content-package-maven-plugin的ui.content和ui.apps内容包安装到包管理器中，以在localhost（端口4502）上默认创作实例。 主机名和端口可以使用`aem.host`和`aem.port`用户定义的属性进行更改。 |
...
| `autoInstallSinglePackage` | 将`all`内容包与content-package-maven-plugin一起安装到包管理器，以在localhost上的默认作者实例（端口4502）。 主机名和端口可以使用`aem.host`和`aem.port`用户定义的属性进行更改。 |
| `autoInstallSinglePackagePublish` | 将包含content-package-maven-plugin的`all`内容包安装到包管理器，以在localhost上的默认发布实例（端口4503）。 主机名和端口可以使用`aem.host`和`aem.port`用户定义的属性进行更改。 |
...
如果您有正在运行的AEM实例，则可以使用以下Maven命令构建并打包整个项目并部署到AEM中。
...
项目(`<src-directory>/<project>/pom.xml`)根部的`pom.xml`称为父POM，它驱动项目的结构并管理依赖项和项目的某些全局属性。

### 全局项目属性{#global-properties}

父POM的`<properties>`部分定义了对在AEM实例上部署项目非常重要的几个全局属性，如用户名/密码、主机名/端口等。

这些属性设置为部署到本地AEM实例，因为这是开发人员将执行的最常见的构建。 请注意，存在要部署到作者实例和发布实例的属性。 此外，还将凭据设置为与AEM实例进行身份验证。 使用默认的admin:admin凭据。

设置这些属性，以便在部署到更高级别环境时可以覆盖它们。 这样，POM文件就不必更改，但诸如`aem.host`和`sling.password`之类的变量可以通过命令行参数覆盖：
...
父POM的`<modules>`部分定义项目将构建的模块。 默认情况下，项目会生成[以前定义的标准模块](#what-you-get):core、ui.apps、ui.content、ui.tests和it.launcher。 随着项目的不断发展，可以始终添加更多模块。

### 依赖关系 {#dependencies}

父POM的`<dependencyManagement>`部分定义项目中使用的所有API的依赖项和版本。 应在父POM中管理版本。 核心和ui.apps等子模块不应包含任何版本信息。
...
>[!NOTE]
>
>每个版本的核心组件通常随后都会发布AEM项目原型，以便最新原型使用最新版本的核心组件。
>
>但是，原型的新版本可能不会直接遵循核心组件的新版本，因此您可能希望将核心组件的依赖关系更新到最新版本。

>[!NOTE]
>
>core.wcm.components.examples是一组示例页，用于说明核心组件的示例。 作为最佳实践，在部署用于生产的项目时，应删除此依赖项和子包包含。

## 测试 {#testing}

项目中包含三个测试级别，由于它们是不同类型的测试，它们以不同的方式或在不同位置执行。

* 核心单元测试：它展示捆绑包中包含的代码的经典单元测试。 要测试，请执行：
   * `mvn clean test`
* 服务器端集成测试：这些测试在AEM-环境(即在AEM服务器上)中运行类似于单位的测试。 要测试，请执行：
   * `mvn clean verify -PintegrationTests`
* 客户端Hobbes.js测试：这些是基于JavaScript的浏览器端测试，用于验证浏览器端行为。 要测试：
...
因此，您已经构建并安装了AEM Project Archetype。 现在呢？ 原型很小，但包含许多根据推荐最佳实践配置的强大AEM功能示例。 使用这些功能可以指示您如何在项目中利用这些功能。 对于任何您可能需要：

* [通过扩展现有核心组件自定义组件](/help/developing/customizing.md)
