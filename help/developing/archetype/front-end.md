...
无论您决定为项目实施哪种前端开发工作流，后端开发人员和前端开发人员都必须首先共同确定标记。 通常，AEM会定义由核心组件提供的标记。[但是，如有必要，可以自定义它。](/help/developing/customizing.md#customizing-the-markup)
...
可使用[AEM clientlib.](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/implementing/developing/full-stack/clientlibs.html?lang=zh-Hans)获取前端模块。在执行NPM生成脚本时，将生成应用程序，`aem-clientlib-generator`包将获取生成的生成输出并将其转换为此类clientlib。
