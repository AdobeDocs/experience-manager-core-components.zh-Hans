...
该组件可用于收集年龄、数量等数值数据，也可用于执行加法和减法等数学运算。该组件还可用于验证用户输入的数值数据。
...
* **数据范围**：数值字段可用于通过使用 min、max 和 step 属性来设置有效值的范围。
...
2023 年 2 月，作为核心组件 2.0.4 的一部分发布了自适应表单数值输入核心组件。下表展示所有支持的版本、AEM 兼容性和相应文档的链接：
...
* **显示格式** - 此选项可让您从不同的整数数值类型格式中选择选项来进行显示。当用户从&#x200B;**类型**&#x200B;下拉菜单中选择任意选项时，**格式**&#x200B;选项将在面板中可见。您可以选择向用户显示的数字所采用的特定格式。
...
“设计”对话框用于定义和管理组件的 CSS 样式。自适应表单数值输入核心组件支持 AEM [样式系统](/help/get-started/authoring.md#component-styling)。
