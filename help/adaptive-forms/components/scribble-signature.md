---
title: 自适应表单核心组件 - 潦草签名
description: 使用或自定义自适应表单潦草签名核心组件。
...
- **A：**&#x200B;点击&#x200B;**绘制**&#x200B;图标可在画布上绘制您的签名。
- **B：**&#x200B;点击&#x200B;**清除**&#x200B;图标可清除画布上的签名。
- **C：**&#x200B;点击&#x200B;**地理位置**&#x200B;图标可一起添加地理位置和签名。
- **D：**&#x200B;点击&#x200B;**键盘**&#x200B;图标可在画布上输入您的名字。

在潦草签名窗口中选择&#x200B;**保存**&#x200B;图标后，您就无法再编辑这个签名。如果您想编辑签名，必须忽略当前签名，使用上面的画笔/键盘选项重新签名。
...
在表单中加入潦草签名很有好处，这可能有若干原因，包括：

- **数字同意**：允许用户提供具有法律效力的电子签名。
- **改善用户体验**：提供一种可直接在设备上自然签名的方式，无需扫描或上传。
- **无纸工作流**：避免打印、签名和重新扫描文档。
- **身份验证**：作为额外的确认和审批层。
- **数据准确性**：确保以数字形式正确捕捉签名者的手写输入。

## 版本和兼容性 {#version-and-compatibility}

自适应表单潦草签名核心组件于 **2025 年 8 月**&#x200B;作为 Cloud Service 的&#x200B;**核心组件 2.24.6** 及更高版本的一部分发布。
...
在 [GitHub](https://github.com/adobe/aem-core-forms-components) 上获取有关自适应表单潦草签名核心组件的最新技术详细信息。有关核心组件开发的更多信息，请参阅[核心组件开发人员文档](/help/developing/overview.md)。
...
- **签名对话框标题**：签名对话框标题定义了在签名捕捉对话框顶部显示的文本。在需要用户提供签名时，它起到提示或说明的作用。文本能帮助引导用户完成签名过程，使交互更清晰直观。

### “验证”选项卡

![“验证”选项卡](/help/adaptive-forms/assets/scribble-signature-validation.png)
...
“设计”对话框用于定义和管理潦草签名组件的 CSS 样式。

### “样式”选项卡 {#styles-tab}

该选项卡用于定义和管理组件的 CSS 样式。自适应表单潦草签名核心组件支持 AEM [样式系统](/help/get-started/authoring.md#component-styling)。

![“设计”对话框](/help/adaptive-forms/assets/checkbox-style.png)

- **默认 CSS 类**：您可为自适应表单潦草签名核心组件提供一个默认 CSS 类。
