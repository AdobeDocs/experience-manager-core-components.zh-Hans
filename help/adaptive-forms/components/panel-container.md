...
在自适应表单中，面板是一个容器元素，可用于组合相关表单元素。它可让您以一种符合逻辑且有意义的方式对不同的表单元素进行分组和整理。这有助于改善表单的整体结构和可读性，使用户能够更轻松地理解和导航。

面板可用于创建可折叠部分，这对于隐藏复杂或不常用的表单字段来说很有用，可以使表单简单易用。通过它还可包括其他组件，如文本、复选框、按钮。
...
该选项卡用于定义和管理组件的 CSS 样式。自适应表单面板容器核心组件支持 AEM [样式系统](/help/get-started/authoring.md#component-styling)。

![“样式”选项卡](/help/adaptive-forms/assets/panel_style.png)

* **默认 CSS 类**：可为自适应表单核心组件提供默认 CSS 类。
