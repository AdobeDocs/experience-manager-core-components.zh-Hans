...
复选框通常表示为一个小方块，可通过单击或点按它来将其打开或关闭。选中复选框后，将显示一个复选标记，以表明关联的选项或功能已激活或已启用。
...
- **可视确认**：选中的复选框可让用户从视觉上确认其选择已被记录。此反馈有助于防止用户错误，并确保他们知道他们的选择已登记。

- **错误预防**：复选框允许用户在提交表单之前检查并确认选择，从而降低出错概率。

## 版本和兼容性 {#version-and-compatibility}

自适应Forms复选框核心组件作为核心组件2.0.52的一部分发布。下表显示了所有支持的版本、AEM兼容性以及相应文档的链接：

|  |  |
|---|---|
| 组件版本 | AEM as a Cloud Service |
| --- | --- |
| v1 | 兼容<br>[版本 2.0.52](/help/versions.md) 和更高版本 | 兼容 | 兼容 |
...
- **在选中时，返回值** - 选中此选项可指定在选中复选框时应与复选框关联的值。这是在您标记或勾选复选框时发生的操作。
- **启用取消选中。**- 选中该选项可启用或禁用取消选中之前已选中的复选框的功能。
   - 如果&#x200B;**启用取消选中**&#x200B;已启用或设置为 true，这意味着用户可以自行决定选中和取消选中该复选框。他们可以根据需要选中和取消选中该复选框。

   - 如果&#x200B;**启用取消选中**&#x200B;已禁用或设置为 false，这意味着一旦选中该复选框，用户便无法将其取消选中。
...
“自定义属性”对话框可让您使用表单模板将自定义属性（键值对）关联到自适应表单核心组件。自定义属性反映在组件 Headless 演绎版的属性部分中。它可让您创建根据自定义属性值进行调整的动态表单行为。例如，开发人员可以为移动、桌面或 Web 平台设计 Headless 表单组件的各种演绎版，从而大大增强各种设备上的用户体验。

- **组名称**：您可以提供名称来标识自定义属性组。您可以添加、删除或重新排列多个自定义属性组。添加自定义属性组后，可看到以下选项：

   - **键值对**：您可以通过为每个自定义属性组单击&#x200B;**添加**&#x200B;按钮，来添加多个自定义属性名称和自定义属性值。
