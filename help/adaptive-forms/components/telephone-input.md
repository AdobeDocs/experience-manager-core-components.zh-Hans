...
2023 年 2 月，作为核心组件 2.0.4 的一部分发布了自适应表单电话号码输入核心组件。下表展示所有支持的版本、AEM 兼容性和相应文档的链接：
...
**验证模式**&#x200B;选项可让您输入一个模式来验证输入的电话号码。根据在&#x200B;**模式**&#x200B;选项中输入的值验证输入的电话号码。如果电话号码未能通过使用&#x200B;**模式**&#x200B;选项中输入的值进行的验证，则屏幕上会显示错误消息。

* **图案**  — 此选项允许您输入允许的电话号码验证模式。 也可以使用正则表达式。
...
“设计”对话框用于定义和管理组件的 CSS 样式。自适应表单电话号码输入核心组件支持 AEM [样式系统](/help/get-started/authoring.md#component-styling)。
