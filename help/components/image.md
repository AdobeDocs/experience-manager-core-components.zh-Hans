...
下表详细说明了该组件的所有受支持版本、与该组件的版本兼容的AEM版本以及指向早期版本文档的链接。
...
图像组件提供了可靠的响应式功能，可以直接使用。在页面模板级别，可以使用[“设计”对话框](#design-dialog)来定义图像资源的默认宽度。图像组件会自动加载正确的宽度，以根据浏览器窗口的大小进行显示。 在窗口调整大小时，图像组件即时动态加载正确的图像大小。组件开发人员无需担心自定义媒体查询的定义方式，因为图像组件已经针对加载内容进行了优化。

此外，图像组件支持延迟加载，可以将实际图像资源的加载推迟到在浏览器中可见时，从而提升了页面的响应能力。

>[!TIP]
>
>默认情况下，图像组件由自适应图像 Servlet 提供支持。 请参阅 [自适应图像Servlet](/help/developing/adaptive-image-servlet.md) 了解其工作原理的详细信息。
...
* 从DAM拖放SVG资源以及从本地文件系统上传的SVG文件均受支持。
...
要体验图像组件并查看其配置选项示例以及HTML和JSON输出，请访问 [组件库](https://adobe.com/go/aem_cmp_library_image_cn).
...
取决于您是否拥有 [Dynamic Media](#dynamic-media) 已启用或 [新一代Dynamic Media](#next-gen-dm) 功能启用后，可用于编辑图像的选项会有所不同。
...
>[!NOTE]
>
>GIF 图像不支持编辑图像的操作。在编辑模式下对GIF所做的任何此类更改都不会保留。
...
* **图像资源** - 如果选中了&#x200B;**从页面继承特色图像**，则自动填充此项。取消选中可通过设置以下选项而手动定义图像。
   ...
   * 将资源从 [资产浏览器](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/sites/authoring/fundamentals/environment-tools.html) 或点按 **浏览** 选项，以便您可以从本地文件系统上传。
   * 点按或单击&#x200B;**“清除”**&#x200B;以取消选择当前选定的图像。
   * 点击或单击 **选取** 以打开 [资产浏览器](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/sites/authoring/fundamentals/environment-tools.html) 以便您选择图像。
...
* **不提供替换文本**  — 将图像标记为被屏幕阅读器等辅助技术忽略，以用于图像纯粹起装饰作用或不向页面传达额外信息的情况。
...
* **预设类型** – 这定义了可用图像预设的类型，可以是&#x200B;**图像预设**&#x200B;或&#x200B;**智能裁切**，并且仅在启用了 [Dynamic Media 功能](#dynamic-meida)的情况下可用。
   ...
   * **智能裁剪**  — 时间 **预设类型** 之 **智能裁剪** 已选中，下拉菜单 **节目** 可用，并允许从所选资源的可用演绎版中进行选择。 只有为所选资源定义了演绎版，才有此项可用。
   * **图像修饰符** – 在这里可以定义其他 Dynamic Media 图像服务命令，不论选择了什么&#x200B;**预设类型**，都使用 `&` 分隔。
* **题注** – 有关图像的附加信息，默认显示在图像的下方。
   * **从DAM获取描述**  — 选中时，图像的描述文本将使用 `dc:title` dam中的元数据。
   * **以弹出形式显示描述**  — 选中后，不会在图像下方显示描述，但在某些浏览器中，将鼠标悬停于图像上方时，会以弹出信息形式显示描述。
* **链接** – 将图像链接到其他资源。
   * 使用“选择”对话框可链接到其他 AEM 资源。
   * 如果未链接到 AEM 资源，请输入绝对 URL。非绝对URL将解释为相对于AEM。
   * **在新标签页中打开链接** – 此选项在一个新的浏览器窗口中打开链接。
* **ID**  — 利用此选项，可控制HTML和中的组件的唯一标识符。 [数据层](/help/developing/data-layer/overview.md).
   * 如果留空，则自动为您生成一个唯一 ID，通过检查所得页面即可找到该 ID。
   * 如果指定一个 ID，作者有责任确保它是唯一的。
   * 更改此ID会对CSS、JS和Data Layer跟踪产生影响。

>[!TIP]
>
>**智能裁切**&#x200B;和&#x200B;**图像预设**&#x200B;为互斥选项。如果作者必须使用图像预设以及智能裁剪演绎版，则作者必须使用 **图像修饰符** 以手动添加预设。
...
必须在中为此组件配置样式 [“设计”对话框](#design-dialog) ，以便下拉菜单可用。
...
* **启用Web优化图像**  — 选中后， [web优化图像投放服务](/help/developing/web-optimized-image-delivery.md) 以WebP格式发送图像，平均将图像大小减少25%。
   * 此选项仅在 AEMaaCS 中可用。
   * 当未选中或Web优化图像投放服务不可用时， [自适应图像Servlet](/help/developing/adaptive-image-servlet.md) 已使用。
* **禁用延迟加载**  — 选中后，组件预加载所有图像而不会延迟加载。
...
* **宽度** – 定义图像的宽度（以像素为单位），组件将根据浏览器的大小自动加载最合适的宽度。
   ...
   * 默认情况下，图像加载会被延迟，直到图像变得可见。
      * 选择选项 **禁用延迟加载** 这样您就可以在页面加载时加载图像。
* **JPEG质量**  — 转换（例如，缩放或裁切）的JPEG图像的质量因子（以从0到100的百分比表示）。
