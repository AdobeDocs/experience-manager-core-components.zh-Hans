...
使用核心组件轮播组件，内容作者可在可导航的轮播中展示内容。

{{traditional-aem}}

## 用途 {#usage}

使用轮播组件，内容作者可以将内容排列在旋转的幻灯片轮播中。

内容作者可以使用[“编辑”对话框](#edit-dialog)创建、命名和排序多个幻灯片，以及启用带有延迟的自动过渡。 使用[“设计”对话框](#design-dialog)，模板作者可以定义哪些组件可以添加到轮播中，启用或禁用自动过渡，以及自定义样式。

## 版本和兼容性 {#version-and-compatibility}

轮播组件的当前版本是 V1，此版本随 2018 年 10 月的核心组件发行版本 2.2.0 的发布引入，具体说明见本文。
...
要体验轮播组件并查看其配置选项示例以及 HTML 和 JSON 输出，请访问[组件库](https://adobe.com/go/aem_cmp_library_carousel_cn)。

### 技术详细信息 {#technical-details}

[在 GitHub 上可找到](https://adobe.com/go/aem_cmp_tech_carousel_v1_cn)有关轮播组件的最新技术文档。

在[核心组件开发人员文档](/help/developing/overview.md)中可找到有关开发核心组件的其他详细信息。

## 深度链接到面板 {#deep-linking}

轮播组件、[选项卡组件](tabs.md)和[可折叠项组件](accordion.md)支持直接链接到组件中的面板。
...
![轮播组件“编辑”对话框的“项目”选项卡](/help/assets/carousel-edit-items.png)
...
![轮播组件“编辑”对话框的“属性”选项卡](/help/assets/carousel-edit-properties.png)
...
* **禁止在光标悬停时自动暂停** - 选择了&#x200B;**自动过渡幻灯片**&#x200B;时，只要光标悬停在轮播上时，轮播过渡将自动暂停。 选择此选项可不暂停过渡。
...
![轮播组件“编辑”对话框的“辅助功能”选项卡](/help/assets/carousel-edit-accessibility.png)

在&#x200B;**辅助功能**&#x200B;选项卡上，可以为组件的 [ARIA 辅助功能](https://www.w3.org/WAI/standards-guidelines/aria/)标签设置值。

* **标签** - 轮播的 aria-label 属性的值，它描述轮播的内容
* **上一个** - 轮播导航的上一个按钮标签的 aria-label 属性的值
* **下一个** - 轮播导航的下一个按钮标签的 aria-label 属性的值
* **播放** - 轮播导航的播放按钮标签的 aria-label 属性的值
* **暂停** - 轮播导航的暂停按钮标签的 aria-label 属性的值
* **选项卡列表** - 轮播导航的项目列表标签的 aria-label 属性的值
* **将项目的 aria 标签设置为其标题** - 如果选中此选项，则自动将轮播项目标题设置为其 aria-label 描述。
...
使用“设计”对话框，模板作者可以定义哪些组件可以作为幻灯片添加到轮播组件中，以及定义自动过渡默认值和可供内容作者使用的自定义样式。

### “属性”选项卡 {#properties-tab-1}

在内容作者添加轮播组件到页面时，可以使用&#x200B;**属性**&#x200B;选项卡定义幻灯片过渡的默认设置。

![轮播组件“设计”对话框](/help/assets/carousel-design.png)

* **自动过渡幻灯片** - 定义当内容作者添加轮播组件到页面时，默认是否自动将轮播切换到下一张幻灯片的选项。
* **前置控制元素** - 选中后，控制元素将被放在轮播项目的前面以提高无障碍性。
...
轮播组件支持 AEM [样式系统](/help/get-started/authoring.md#component-styling)。

## Adobe Client Data Layer {#data-layer}

轮播组件支持 [Adobe 客户端数据层](/help/developing/data-layer/overview.md)。
