...
Teaser组件（截至[版本2.23.2](/help/versions.md)）支持远程资产。[配置后，](/help/developing/remote-assets.md)您可以从远程服务为Teaser组件选择资源。
...
Teaser标题、描述和图像可以从链接的页面继承，也可以从在第一个call to action中链接的页面继承。 如果未指定链接或call to action，则将从当前页面继承标题、描述和图像。
...
Teaser 组件将图像渲染委派给[图像组件](image.md)。 因此，内容作者可以使用图像组件的[“编辑”对话框](image.md#edit-dialog)来处理 Teaser 图像。
