...
图像组件具有适应页面访问者的自适应图像选择和响应式行为，以及内容作者轻松的图像放置和裁剪。
...
图像组件提供现成的强大响应式功能。在页面模板级别， [设计对话框](#design-dialog) 可用于定义图像资产的默认宽度。然后，图像组件会根据浏览器窗口的大小自动加载正确的宽度。调整窗口大小时，“锯齿组件”会动态加载正确的图像大小。组件开发人员无需担心定义自定义媒体查询，因为图像组件已经优化以加载内容。

此外，图像组件还支持延迟加载实际图像资产，直到它在浏览器中可见，从而提高页面的响应性。
...
由于安全原因，图像编辑器从不直接调用原始SVG。调用它 `<img src=“path-to-component”>`。这可防止浏览器执行嵌入到SVG文件中的任何脚本。
...
**在主** 选项卡上，您可以为图像定义宽度列表(以像素为单位)，并且组件将根据浏览器大小自动加载最合适的宽度。这是图像组件 [响应式功能](#responsive-features) 的重要部分。
...
* **宽度**&#x200B;为图像定义宽度(以像素为单位)列表，并且组件根据浏览器大小自动加载最合适的宽度。
   * 点按或单击 **添加** 按钮以添加其他大小。
      * 使用抓取手柄重新排列大小的顺序。
      * 使用 **删除** 图标删除宽度。
   * 默认情况下，加载图像时会延迟加载图像。
